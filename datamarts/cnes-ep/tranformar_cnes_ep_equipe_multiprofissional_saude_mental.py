...
def sumarizar_arquivos():
    ...
    df = df[(df["TIPO_EQP"] == 58) | (df["TIPO_EQP"] == 59) | (df["TIPO_EQP"] == 60) | (df["TIPO_EQP"] == 75)]
    ...
    result.to_csv('silver/cnes-ep-equipe-multiprofissional-saude-mental.csv', sep=';', index=False)
...
def agrupar_arquivos():
    ...
    # Get a list of all the CSV files in the directory that start with 'cnes'
    csv_files = [f for f in os.listdir(path) if
                 f.startswith('cnes-ep-equipe-multiprofissional-saude-mental') and f.endswith('.csv')]
    ...
    output_file = os.path.join('gold/', 'cnes-ep-equipe-multiprofissional-saude-mental.csv')
